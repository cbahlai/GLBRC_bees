#####################################################
#
#     Analysis of GLBRC 2008-9 bee data- monthly analysis
#
#
...
#create a month variable in the specimen list
bee.specimens$Month<-months(as.Date(bee.specimens$DOY, origin="2008-01-01"))
colnames(bee.specimens)

# melt data
specimen.list<-melt(bee.specimens, id=c("State", "Year","Month","DOY", "Family", "Genus","Species","Taxon",
                                        "Group" ,"Site.ID", "Sample","Treatment" ,"Trap.height","Nest.biology",
                                        "Sociality", "LAT" , "LON"  ), na.rm=TRUE)
...
species.list.bymonth<-dcast(specimen.list, Taxon~Month, length)
...
metrics.matrix<-dcast(specimen.list, Year+Month+State+Site.ID+Treatment~., length)

#cast a cross-tab  by species
bee.matrix<-dcast(specimen.list, Year+Month+State+Site.ID+Treatment~Taxon, length)
...
#bee.matrix$Treatment<-factor(bee.matrix$Treatment, levels=c("corn", "switchgrass","prairie"))
#also  remove site, year columns from data (required for diversity computations)
bee.matrix$Year<-NULL
bee.matrix$Month<-NULL
bee.matrix$State<-NULL
bee.matrix$Site.ID<-NULL
bee.matrix$Treatment<-NULL
# treat honey bees separately
metrics.matrix$Apis.mellifera<-bee.matrix$Apis.mellifera
bee.matrix$Apis.mellifera<-NULL
metrics.matrix$Bee.abundance<-rowSums(bee.matrix)

#cast a cross-tab with counts by pollen deposition group
group.matrix<-dcast(specimen.list, Year+Month+State+Site.ID+Treatment~Group, length)
#develop total pollen deposition estimate based on winfree 2007 simulations
#and put it in the oerall metrics matrix
#do not include honey bees in estimate
metrics.matrix$pollen.dep<-2814*group.matrix$Bumble+1515*group.matrix$Small+624*group.matrix$Green+94*group.matrix$Large

#cast a cross-tab with counts by nest biology
nest.matrix<-dcast(specimen.list, Year+Month+State+Site.ID+Treatment~Nest.biology, length)
# Quick PCA to reduce dimensionality of nesting biology data
nest.pca<-prcomp(nest.matrix[6:11], scale.=T)
biplot(nest.pca)
summary(nest.pca)
...
library(Hmisc)
rcorr(as.matrix(nest.matrix[6:11]), type="pearson")

#cast a cross-tab with counts by sociality
sociality.matrix<-dcast(specimen.list, Year+Month+State+Site.ID+Treatment~Sociality, length)
sociality.pca<-prcomp(sociality.matrix[6:9], scale.=T)
...
rcorr(as.matrix(sociality.matrix[6:9]), type="pearson")
...
metrics.matrix<-merge(metrics.matrix, group.matrix, by=c("Year","Month", "State", "Site.ID", "Treatment"))
metrics.matrix<-merge(metrics.matrix, nest.matrix, by=c("Year","Month", "State", "Site.ID", "Treatment"))
metrics.matrix<-merge(metrics.matrix, sociality.matrix, by=c("Year","Month","State", "Site.ID", "Treatment"))
...
abundance.model<-glm.nb(Bee.abundance~as.factor(Year)+Month+Treatment+Annual+Perennial+Forest+Urban+Wetland+OpenWater, na.action=na.fail, data=metrics.matrix)
...
pollen.model<-glm.nb(pollen.dep~as.factor(Year)+Month+Treatment+Annual+Perennial+Forest+Urban+Wetland+OpenWater, na.action=na.fail, data=metrics.matrix)
...
apis.model<-glm.nb(bee.matrix$Apis.mellifera~as.factor(Year)+Month+Treatment+Annual+Perennial+Forest+Urban+Wetland+OpenWater,  na.action=na.fail, data=metrics.matrix)
...
#exclude location months with fewer than  10 bees captured
...
H.model<-glm(H~as.factor(Year)+Month+Treatment+Annual+Perennial+Forest+Urban+Wetland+OpenWater,  na.action=na.fail, data=metrics.matrix.new)
...
D.model<-glm(simp~as.factor(Year)+Month+Treatment+Annual+Perennial+Forest+Urban+Wetland+OpenWater,  na.action=na.fail, data=metrics.matrix.new)
...
S.model<-glm(Srare~as.factor(Year)+Month+Treatment+Annual+Perennial+Forest+Urban+Wetland+OpenWater,  na.action=na.fail, data=metrics.matrix.new)
...
plotmeans(Apis.mellifera~Treatment, cex=2, data=metrics.matrix, xlab="", ylab="Honey bee abundance", n.label=FALSE, barcol="black", pch=16)
...
metrics.matrix$Month<-factor(metrics.matrix$Month, levels=c("June", "July","August"))
...
ord.groups<-metaMDS(metrics.matrix[10,11,13,14])
ord.groups
...
ord.nest<-metaMDS(metrics.matrix[15:20])
ord.nest
...
ord.soc<-metaMDS(metrics.matrix[21:24])
ord.soc
